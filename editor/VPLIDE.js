		...
		VPL_IDE = function(root_id, options) {
			...
			function resultToHTML(text) {
				...
				var lastAnotationFile = false;
                                html = '<meta name="viewport" content="width=device-width, initial-scale=1"> <link rel="stylesheet" href="bootstrap/css/bootstrap.min.css"> <script src="jquery/jquery-1.9.1.js"></script> <script src="bootstrap/js/bootstrap.min.js"></script>';
				...
				function getComment() {
					...
					var res = '';
					var ncase = 1;
					// objeto com os atributos de cada caso de teste
					var case_info = {info:'Info', text:'Dica', input:'Entrada', output:'Saída', expected:'Esperado'};
					res = '<table class="table table-condensed" style="border-collapse:collapse;max-width: 400px">';
					res = res + '<thead class="thead-default"><tr><th>Teste</th><th>Nota</th></tr></thead><tbody>';
					// gera uma linha para cada teste	
					for (k in obj) {
					  res = res + '<tr data-toggle="collapse" data-target="#'+k+'"><td>'+k+'</td><td>'+obj[k].grade+'</td></tr>';
					  res = res + '<tr><td colspan="12" class="hiddenRow"><div class="accordian-body collapse" id="'+k+'">';
					  res = res + '<table class="table table-bordered table-striped"><thead><tr><th>Caso</th><th>Status</th><th>Penalidade</th></tr></thead>';
					    res = res + '<tbody>';
					  var casos = obj[k].cases;
                                          // gera uma linha para cada caso de teste
					  for (data in casos) {
					    var nota = 0;
					    var status = '';
					    if (! casos[data].success) {
					      nota = casos[data].reduction;
					      status = 'Falha';
					    } else status = 'Sucesso';
					  ...
					  res = res + '</tbody></table></div></td></tr>';

					}
					res = res + '</tbody></table>';
					/*for (k in obj) {
					  res = res + '<a data-toggle="collapse" data-target="#'+k+'">Teste '+k+'</a><br>';
					  res = res + '<div id="'+k+'" class="collapse">';
					  for (data in obj[k]) {
						//res = res + '<a data-toggle="collapse" data-target="#demo'+col_cnt+'">'+data+'</a><div id="demo'+col_cnt+'" class="collapse"><table class="navbox" style="text-align: left; border: 1px solid silver; margin-top: 0.2em;"><tr><td style="border: solid 1px silver; padding: 8px; background-color: white;">'+obj[k][data]+'</td></tr></table></div><br>';
						res = res + '<a data-toggle="collapse" data-target="#demo'+col_cnt+'">'+data+'</a><div id="demo'+col_cnt+'" class="collapse"><table class="collapse navbox" style="text-align: left; border: 1px solid silver; margin-top: 0.2em;"><tr><td style="border: solid 1px silver; padding: 8px; background-color: white;">'+obj[k][data]+'</td></tr></table></div><br>';
						col_cnt = col_cnt + 1;
					  }
					  res = res + '</div><br>';
					}*/
					...
					function genCol(data, index) {
						test = obj[data]
						res = res + '<a data-toggle="collapse" data-target="#demo'+col_cnt+'">'+data+'</a><div id="demo'+col_cnt+'" class="collapse"><table class="navbox" style="text-align: left; border: 1px solid silver; margin-top: 0.2em;"><tr><td style="border: solid 1px silver; padding: 8px; background-color: white;">'+data+'</td></tr></table></div><br>';
						col_cnt = col_cnt + 1;
					} 
					obj.keys().forEach(genCol);
					//ret = '<a data-toggle="collapse" data-target="#demo'+col_cnt+'">[+]</a><div id="demo'+col_cnt+'" class="collapse"><table class="navbox" style="text-align: left; border: 1px solid silver; margin-top: 0.2em;"><tr><td style="border: solid 1px silver; padding: 8px; background-color: white;">'+ret+'</td></tr></table></div>'
					//col_cnt = col_cnt + 1;
					return res;
				}
